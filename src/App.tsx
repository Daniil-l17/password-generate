...
function App() {
  ...
  const [activeTabs, setActiveTabs] = useState(tabs.slice(0,2).map(el => el.value))



  const generatePassword = () => {
    const shard = activeTabs.join('')
    let password = ''
    for(let i=0;i < length;i++){
      password += shard[Math.floor(Math.random() * shard.length)]
    }
    ...
    <div className="flex min-h-screen items-center justify-center text-white">
      <div className="bg-gray-700 p-4 rounded-xl min-w-96">
        ...
        <div className="bg-gray-600 flex border border-gray-500 justify-between items-center mt-2 gap-3 p-3 rounded-xl">
          ...
          <button
            onClick={() => setLength((prev) => (prev >= 20 ? prev : prev += 1))}
            className="bg-gray-500 active:scale-95 transition w-9 h-9 rounded-full"
          >
            +
          </button>
        ...
        <div className="flex mt-2 gap-1">
          {password.length > 0 && password.length < 5 ? (
              ...
              <>
                <div className="w-full h-1.5 rounded-full bg-green-500" />
                <div className="w-full h-1.5 rounded-full bg-green-500" />               <div className="w-full h-1.5 rounded-full bg-green-500" />               <div className="w-full h-1.5 rounded-full bg-green-500" />
              </>
          ) : (
              <>             <div className="w-full h-1.5 rounded-full bg-gray-500" />
                <div className="w-full h-1.5 rounded-full bg-gray-500" />
        ...
        <div className="flex mt-3 border border-gray-500 shadow-md rounded-xl overflow-hidden">
          {tabs.map((tab, id) => (
              <button
                  onClick={() => {
                    activeTabs.includes(tab.value) ? setActiveTabs(prev => prev.filter(t => t !== tab.value)) : setActiveTabs(prev => [...prev,tab.value])
                  }}
                  className={`
